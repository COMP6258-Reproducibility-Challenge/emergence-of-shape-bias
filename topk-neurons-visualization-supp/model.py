...
class Model:
    def __init__(
        ...
        layer_weights: List[float] = [1e09, 1e09, 1e09, 0, 0],
        #2 layer_weights: List[float] = [1e09, 0, 1e09, 0, 1e09],
        #3 layer_weights: List[float] = [0, 0, 0, 1e09, 0],
        #4 layer_weights: List[float] = [0, 0, 1e09, 0, 0],
        #5 layer_weights: List[float] = [0, 1e09, 0, 0, 0],
...
class GramHook:
    ...
    def __call__(
        ...
        # sparsity layer out
        layer_out = sparse_hw(layer_out, self.topk, reverse=self.reverse_topk, device='cpu')
...
def sparse_hw(x, topk, reverse=False, device='cpu'):
    n, c, h, w = x.shape
